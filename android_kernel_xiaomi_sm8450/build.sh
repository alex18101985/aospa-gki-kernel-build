...
grep -E "CONFIG_LTO_CLANG=y|CONFIG_LTO_CLANG_THIN=y|CONFIG_CC_OPTIMIZE_FOR_PERFORMANCE=y|CONFIG_DEBUG_KERNEL=y|CONFIG_TRACEPOINTS=y|CONFIG_EVENT_TRACING=y|CONFIG_FTRACE=y|CONFIG_DYNAMIC_FTRACE=y" out/.config | wc -l && \
echo "== Should be disabled but enabled ==" && \
grep -E "CONFIG_LTO_NONE=y|CONFIG_LTO_CLANG_FULL=y|CONFIG_CC_OPTIMIZE_FOR_SIZE=y|CONFIG_DEBUG_INFO=y|CONFIG_DEBUG_PREEMPT=y|CONFIG_DEBUG_RT_MUTEXES=y|CONFIG_DEBUG_SPINLOCK=y|CONFIG_DEBUG_MUTEXES=y|CONFIG_DEBUG_ATOMIC_SLEEP=y|CONFIG_DEBUG_LIST=y|CONFIG_DEBUG_VM=y|CONFIG_DEBUG_OBJECTS=y|CONFIG_SLUB_DEBUG=y|CONFIG_LOCKDEP=y|CONFIG_KFENCE=y|CONFIG_KASAN=y|CONFIG_FUNCTION_TRACER=y|CONFIG_STACK_TRACER=y|CONFIG_FTRACE_SYSCALLS=y" out/.config
